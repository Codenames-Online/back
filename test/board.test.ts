...
describe("Filename: board.test.ts:\n\nThe board", () => {
  it("should have expected number of colors with expected distribution with blue starting team", () => {
    ...
    expect(num_red).to.equal(8);
    expect(num_blue).to.equal(9);
    expect(num_neutral).to.equal(7);
    expect(num_assassins).to.equal(1);
  });

  it("should have expected number of colors with expected distribution with red starting team", () => {
    ...
    expect(board.colors.length).to.equal(25);
    expect(num_red).to.equal(9);
    expect(num_blue).to.equal(8);
    expect(num_neutral).to.equal(7);
    expect(num_assassins).to.equal(1);
  });
